...
def load(setting, start_date, end_date):
    # データ準備
    X, Y = [], []
    x_pattern = ["open", "gradient", "rsi", "rci"]
    y_pattern = ["close"]
    stocks = Loader.stocks()
    stock_data = Loader.loads(stocks["code"], start_date, end_date)
    strategy_setting = strategy.StrategySetting()
    for data in stock_data:
        data = strategy.add_stats(data, strategy_setting)
        X.extend(data[x_pattern].as_matrix())
        Y.extend(data[y_pattern].as_matrix())
    X = numpy.array(X)
    Y = numpy.array(Y)
...
def output_tsv(setting):
    ...
    utils.output_csv(X, "x_train.tsv", sep='\t')
    utils.output_csv(Y, "y_train.tsv", sep='\t')
    ...
    for i, x in enumerate(X):
        print(x)
        utils.output_pgm(x, "%s/x_%s.pgm" % (path, i), 100, 5)
...
def autoencode(setting):
    ...
    X, Y = load(setting)
    utils.output_csv(X, "strategy.csv")
...
def svm(setting):
    X, Y = load(setting)
    x, y, x_test, y_test = utils.validate_split(X, Y, setting.split)
...
def boost(setting, model, output=True):
    X, Y = load(setting)
    x, y, x_test, y_test = utils.under_sampling(X, Y, setting.split)
...
def validate(setting, model):
    X, Y = load(setting)
    x, y, x_test, y_test = utils.under_sampling(X, Y, 0.5)
...
def eval_accuracy(trainer, x, y, timestamp=None, output=True):
    ...
    utils.output_csv(predict, "predict.csv", timestamp)
    utils.output_csv(y, "y_test.csv")
...
if __name__ == "__main__":
  ...
  setting.split = 0.8
  setting.input_neurons = 3
  setting.input_shape = (-1, setting.input_neurons)
  setting.batch_input_shape = (-1, setting.input_neurons, 1)
  setting.output_shape = (-1)
  setting.output_neurons = 1
  setting.hidden_neurons = 512
  ...
  setting.with_batch = False
  model = ModelCreator.dnn
